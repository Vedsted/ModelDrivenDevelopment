# Internal C# DSL for Creating GitLab CI/CD Pipelines
...
## About the DSL
This DSL is meant to ease the creation of GitLab CI pipelines.
...
GitLab_CI_DSL/builder - 
GitLab_CI_DSL/generator - 
...
the DSL consists of the following commands:
  ...
  - `Stage`
    ...
    - The jobs within a stagte will not start before the previous stage is finished
  - `Job`
  ...
  - `Image`
    ...
    - `Image` can be set for default, abstract jobs and jobs
  - `EnvVar`
  ...
  - `Extends`
    ...
    - When extending abstract jobs, the parent configuration will be set first. However these confiurations can be overridden later by other jobs or abstract jobs. (see example 3) 
...
To genrate a GitLab CI pipeline specification, use the `GitLabGenerator` class. This class takes the desired file name, path and a pipeline object and outputs the specification file.
...
The example pipelines will be printed in the console if a invalid output path is defined in the different pipelines.
...
## Example 2 - Maven
Pipeline for maven project with two jobs.
This pipeline will build and test the code.

This is an example of how abstract jobs can be used to specify Docker image and a pre script for both jobs.
...
This is an exmple to show how a job extending multiple abstract jobs works.

The example shows that image and environment variable can be inherited and overridden. 
